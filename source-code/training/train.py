...
if __name__ == '__main__':
    ...
    for epoch in range(epochs):
        ...
        for vi_image, ir_image, name in train_tqdm:
            ...
            fused_img, d1, d2, d3, d4, feats = model(vi_image, ir_image)
            loss_fi_gard = F.l1_loss(gradient(fused_img), torch.max(gradient(vi_image), gradient(ir_image)))
            loss_fi_pix = F.l1_loss(fused_img, torch.max(vi_image, ir_image))
            loss_fi = 50 * loss_fi_gard + 10 * loss_fi_pix
        ...
        torch.save(model.state_dict(), f'{save_path}/teacher.pth')
