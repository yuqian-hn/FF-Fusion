...
class ae_fusion_Net_student(nn.Module):
    def __init__(self, deploy=False):
        ...
        self.deploy = deploy
        # 对不同layer做通道压缩，使得解码时可以对齐
        self.reduce0 = DiverseBranchBlock(8, 8)
        self.reduce1 = DiverseBranchBlock(16, 16)
        self.reduce2 = DiverseBranchBlock(32, 32)
        self.reduce3 = DiverseBranchBlock(64, 64)
        # self.reduce4 = DiverseBranchBlock(2048, 512, deploy=deploy)

        # Decoder
        # self.up4 = nn.ConvTranspose2d(512, 512, kernel_size=2, stride=2)   # from layer4
        self.dec4 = DiverseBranchBlock(64, 64)  # concat with layer3

        self.up3 = nn.ConvTranspose2d(64, 64, kernel_size=2, stride=2)   # from dec4
        self.dec3 = DiverseBranchBlock(96, 32)  # concat with layer2

        self.up2 = nn.ConvTranspose2d(32, 16, kernel_size=2, stride=2)   # from dec3
        self.dec2 = DiverseBranchBlock(32, 16)  # concat with layer1

        self.up1 = nn.ConvTranspose2d(16, 8, kernel_size=2, stride=2)    # from dec2
        self.dec1 = DiverseBranchBlock(16, 8)    # concat with layer0
